...
/**
 * Macro to represent a null/empty data_points_t object.
 *
 * Sets all fields of the data_points_t struct to default empty values:
 * - payload: payload_NULL
 * - width: 0
 * - capacity: 0
 * - nbr_points: 0
 */
#define data_points_NULL ((const data_points_t){.payload = payload_NULL, .width = 0, .capacity = 0, .nbr_points = 0, .init_capacity = 0})

/**
 * The default initial capacity allocated when constructing a new data_points_t object.
 * This can be overridden in data_points_construct().
 */
#define data_points_DEFAULT_INIT_CAP 4096

/**
 * Constructs a new data_points_t object with the given width and initial capacity.
 *
 * Allocates memory for the new data_points_t object and its internal payload.
 * Sets the width, capacity, and initializes nbr_points to 0.
 *
 * @param dtpts Pointer to the data_points_t object to initialize.
 ...
 * @return Pointer to the initialized data_points_t object.
 */
data_points_t *data_points_construct(data_points_t *dtpts, IND_TYP width, IND_TYP init_capacity);

/**
 * Frees the memory allocated for the given data_points_t object.
 *
 * This releases the memory allocated for the data_points_t object's
 * internal payload. It should be called when the object is no longer
 * needed.
 */
void data_points_destruct(data_points_t *dtpts);

// data_points_t *data_points_replace(data_points_t *dest, const data_points_t *src);
// data_points_t *data_points_insert(data_points_t *dest, const data_points_t *src, IND_TYP i);
 ...
 *
 * @param dest The destination data_points_t object to append to.
 * @param src The source data_points_t object to append from.
 * @return A pointer to dest after appending.
 */
data_points_t *data_points_append(data_points_t *dest, const data_points_t *src);

// data_points_t *data_points_append_row(data_points_t *dest, const vec_t *raw_row);
 ...
 */
vec_t *data_points_at(data_points_t *dtpts, vec_t *data, IND_TYP i, const slice_t *dt_sly);
 ...
 * @return A pointer to the output data vector.
 */
vec_t *data_points_at_rnd(data_points_t *dtpts, vec_t *data, const slice_t *dt_sly);

// vec_t *data_points_column_at(data_points_t *dtpts, vec_t *clm, IND_TYP j);

/**
 * Shuffles the data points in the given data_points_t object.
 *
 * @param dtpts The data_points_t object containing the data points to shuffle.
 * @param i_sly The slice indicating which part of the data points to shuffle.
 * @return A pointer to the shuffled data_points_t object.
 */
data_points_t *data_points_shuffle(data_points_t *dtpts, slice_t i_sly);

/**
 * Checks if the given data_points_t object is valid.
 *
 * This checks that the data_points_t object is not null, and that its capacity,
 * number of points, and width fields are valid.
 *
 * @param dtpts The data_points_t object to check.
 * @return True if the data_points_t object is valid, false otherwise.
 */
bool data_points_is_valid(const data_points_t *dtpts);

/**
 * Clears the data points in the given data_points_t object.
 *
 * This resets the number of points to 0 and resets the capacity to init_capacity;
 * The data_points_t object itself is not freed.
 *
 * @param dtpts The data_points_t object to clear.
 * @return A pointer to the cleared data_points_t object.
 */
